 ...
 * </p>
 * <p>
 * <b>That indirection is the whole point, and it is not stylistic.</b> A method declared to return
 * an abstract type — every {@code Provider.provide(int)} in this library — that constructs a
 * subclass forces the verifier to perform an assignability check, and the check loads the subclass.
...
public interface NmsBundle {
    ...
    // ----------------------------------------------------------------------------------------- //
    // The four pre-1.18.2 modules wrap the *shaded* adventure; v_latest delegates to the server's
    // own. Both paths are kept because they are not equivalent — the native serializers are the
    // reason 1.18.2+ is a separate branch today.
